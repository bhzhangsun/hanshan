...
_factory.register('input', props => {
  console.log('input comp props:', props);
  const state = props.findState(props.name);
  return <ElInput {...props} v-model={state[props.name]}></ElInput>;
});
...
_factory.register('container', (props, { slots }) => {
  return <div {...props}>{slots.default ? slots.default() : ''}</div>;
});
